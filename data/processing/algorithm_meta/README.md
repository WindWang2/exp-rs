...
## Capability knowledge layer (D8, ADR 0154) — `capability/`

The subdirectory `capability/` holds the full per-operator capability catalog
(one sidecar per registered `rs:` operator — the guard test pins the set to
the live registry, not to a fixed count) plus the relation graph
...
- Schema, query API, budgets: `src/agent/harness/capability_catalog.h` and
  `docs/adr/0154-capability-relation-graph.md`.
...
- `tests/test_capability_knowledge.cpp` guards coverage (registry set ==
  catalog set), drift, graph invariants, budgets, page regeneration, and
  chain composition. The algorithm/capability-surface parity across
  CLI/MCP/help is gated by `tests/test_capability_surface_parity.cpp`; the
  authored-content completeness contract by
