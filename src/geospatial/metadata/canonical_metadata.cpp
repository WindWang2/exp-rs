    ...
    for ( size_t i = 0; i < dimCount; ++i )
    {
      ...
      if ( GDALMDArrayH indexingVariable = GDALMDArrayH( GDALDimensionGetIndexingVariable( dim ) ) )
      {
        ...
        else if ( axisClass == GEDTC_STRING && info.size > 0 )
        {
          ...
          QuietCplErrors quietAxis;
          // GDAL 3.13 changed GDALMDArrayRead's `count` parameter to
          // size_t (arrayStartIdx stays GUInt64) — mirror the size_t
          // spelling already used by the sibling call above (axisCountSize).
