...
//
// Two views over ONE document (the WorkflowDocument 2.0 AST):
...
class GuidedWorkflowWidget : public QWidget
{
  ...
  public:
    ...
    /// Alternative entry: adopt an already-parsed WorkflowDocument whose
    /// nodes carry is_lab_step / lab_step_index metadata.
    bool setUnderlyingWorkflow( const sicnu::workflow::WorkflowDocument &def );
    ...
    const sicnu::workflow::WorkflowDocument &underlyingWorkflow() const { return m_workflow; }
  ...
  private:
    ...
    sicnu::workflow::WorkflowDocument m_workflow;
