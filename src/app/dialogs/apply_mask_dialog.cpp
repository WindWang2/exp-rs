...
void ApplyMaskDialog::setupUi()
{
  ...
  m_inputLayerCombo = new QComboBox( inputSec );
  ...
  m_maskLayerCombo = new QComboBox( inputSec );
  ...
  QFrame *optSec = SicnuUi::makeSection(
    this, tr( "高级选项" ),
    tr( "默认复用输入波段自身的 NoData；仅在输入波段未定义 NoData 时才需要指定。" ) );
  auto *optForm = new QFormLayout();
  optForm->setContentsMargins( 0, 0, 0, 0 );
  optForm->setHorizontalSpacing( 12 );

  m_useNoDataCheck = new QCheckBox( tr( "指定输出 NoData 值" ), optSec );
  ...
  optForm->addRow( tr( "NoData" ), m_useNoDataCheck );

  m_noDataSpin = new QDoubleSpinBox( optSec );
  ...
  qobject_cast<QVBoxLayout *>( optSec->layout() )->addLayout( optForm );
  mainLayout->addWidget( optSec );
