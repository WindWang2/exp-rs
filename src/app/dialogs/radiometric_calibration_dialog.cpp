...
void RadiometricCalibrationDialog::setupUi()
{
  ...
  QFrame *sec = SicnuUi::makeSection(
    this, tr( "定标参数" ),
    tr( "将 DN 值转换为物理量。传感器元数据（Landsat MTL / Sentinel-2 MTD）"
        "自动从输入栅格旁探测；无元数据时回退到 GDAL scale/offset。" ) );
  auto *form = new QFormLayout();
  form->setContentsMargins( 0, 0, 0, 0 );
  form->setHorizontalSpacing( 12 );
  form->setVerticalSpacing( 8 );

  m_unitCombo = new QComboBox( sec );
  m_unitCombo->addItem( tr( "辐射亮度 Radiance" ), QStringLiteral( "radiance" ) );
  m_unitCombo->addItem( tr( "TOA 反射率" ), QStringLiteral( "toa_reflectance" ) );
  ...
  m_metadataEdit = new QLineEdit( sec );
