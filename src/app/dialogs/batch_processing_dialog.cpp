...
void BatchProcessingDialog::setupUi()
{
  ...
  auto *banner = SicnuUi::makeGroup(
    this, tr( "Batch Processing Pipeline Guide" ),
    tr( "Pick an algorithm → add files to process → set the output directory → start the batch task." ) );
  ...
  bannerLayout->addWidget( SicnuUi::makeHintLabel(
    banner, SicnuDialogHelp::shortForTool(
              QStringLiteral( "batch_processing" ), tr( "Batch Processing" ) ) ) );
  mainLayout->addWidget( banner );

  QGroupBox *algGroup = SicnuUi::makeGroup( this, tr( "Algorithm Selection" ) );
  ...
  m_paramFrame = SicnuUi::makeGroup( this, tr( "Algorithm Parameter Overrides (optional)" ),
                                     tr( "Overrides the algorithm parameters used by the batch run (inputs and outputs are decided by the file list to process)." ) );
  m_paramForm = SicnuUi::makeFormLayout( m_paramFrame );
  m_paramFrame->setVisible( false );
  mainLayout->addWidget( m_paramFrame );

  QGroupBox *fileGroup = SicnuUi::makeGroup( this, tr( "Input File List" ) );
  ...
  SicnuDialogHelp::tip( m_fileList, tr( "List of files to process." ) );
  fileGroupLayout->addWidget( m_fileList );

  auto *fileButtonLayout = new QHBoxLayout();
  fileButtonLayout->setSpacing( 8 );
  m_addFilesBtn = new QPushButton( tr( "Add Files..." ), fileGroup );
  ...
  m_removeBtn = new QPushButton( tr( "Remove Selected" ), fileGroup );
  ...
  QGroupBox *outGroup = SicnuUi::makeGroup( this, tr( "Output Directory Settings" ) );
  ...
  m_outputDirEdit->setPlaceholderText( tr( "Choose the batch result save directory..." ) );
  SicnuDialogHelp::tip( m_outputDirEdit, tr( "All batch run result files are written to this directory." ) );
  connect( m_outputDirEdit, &QLineEdit::textChanged, this, [this]( const QString &text ) {
    m_outputDir = text.trimmed();
  } );
  m_browseBtn = new QPushButton( tr( "Browse..." ), outGroup );
  ...
  m_runButton = buttonBox->addButton( tr( "Run Batch" ), QDialogButtonBox::AcceptRole );
  SicnuUi::markPrimary( m_runButton );
  SicnuDialogHelp::tip( m_runButton, tr( "Runs the processing tasks one by one in list order. Do not force-close the dialog while a batch is running." ) );
...
void BatchProcessingDialog::onAddFiles()
{
    QStringList files = QFileDialog::getOpenFileNames(
        ...
        tr("Remote-Sensing Rasters and Vectors (*.tif *.tiff *.img *.shp *.gpkg);;All Files (*)"));
    ...
    m_statusLabel->setText(tr("%1 input files selected").arg(m_inputFiles.size()));
}
...
void BatchProcessingDialog::onRun()
{
    ...
    if (m_inputFiles.isEmpty()) {
        QMessageBox::warning(this, tr("Batch Processing"),
                             tr("Add the input files to process first."));
        return;
    }
    ...
    if (m_outputDir.isEmpty()) {
        QMessageBox::warning(this, tr("Batch Processing"),
                             tr("Specify the output directory."));
        return;
    }
    ...
    // DLGB-11: ensure output directory exists (typed path may be new).
    {
        ...
        if (!outDir.exists() && !outDir.mkpath(QStringLiteral("."))) {
            QMessageBox::warning(this, tr("Batch Processing"),
                                 tr("Failed to create the output directory:\n%1").arg(m_outputDir));
            return;
        }
    }
...
{
    if (m_canceled) {
        m_batchErrors.append(tr("Batch processing was cancelled by the user"));
        finishBatch();
        return;
    }
...
void BatchProcessingDialog::finishBatch()
{
    ...
    if (m_canceled) {
        m_statusLabel->setText(tr("Batch processing cancelled: %1 succeeded, %2 failed")
                                   .arg(m_batchSuccess).arg(m_batchFail));
    } else {
        m_statusLabel->setText(tr("Batch processing finished: %1 succeeded, %2 failed")
                                   .arg(m_batchSuccess).arg(m_batchFail));
    }

    if (m_batchFail > 0) {
        QString details = tr("Batch processing finished (with some failures):\n%1 succeeded, %2 failed")
                              .arg(m_batchSuccess).arg(m_batchFail);
        if (!m_batchErrors.isEmpty()) {
            const int maxShow = 8;
            details += QLatin1Char('\n') + m_batchErrors.mid(0, maxShow).join(QLatin1Char('\n'));
            if (m_batchErrors.size() > maxShow)
                details += tr("\n... and %1 more errors (see the system log)").arg(m_batchErrors.size() - maxShow);
        }
        QMessageBox::warning(this, tr("Batch Processing"), details);
    } else {
        QMessageBox::information(this, tr("Batch Processing"),
                                 tr("Batch processing finished:\n%1 files processed successfully")
                                     .arg(m_batchSuccess));
    }
}
...
void BatchProcessingDialog::updateAlgorithmParameters()
{
    ...
    if (algorithmId.startsWith(QStringLiteral("rs:"))) {
        ...
        if (adapter) {
            ...
            m_statusLabel->setText(tr("Selected: %1 (RS, default parameters)").arg(
