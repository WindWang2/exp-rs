...
void QgisDesktopWindow::openProcessingAlgorithm(const QString &algorithmId)
{
    ...
    // Use the proper QGIS processing dialog with widget wrappers
    auto *dlg = new SicnuAlgorithmDialog(this);
    dlg->setAlgorithm(algorithm.release()); // dlg takes ownership
...
{
    // Get raster layer: canvas current → tree selection → first raster in project
