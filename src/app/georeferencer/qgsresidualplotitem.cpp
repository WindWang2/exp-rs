/***************************************************************************
    ...
    SICNU port (2026-06-03, Task 11.5.2): adapted to operate on a
    snapshot QVector<Entry> rather than the upstream QList<QgsGeorefDataPoint*>.
 ***************************************************************************
 *                                                                         *
...
QgsResidualPlotItem::~QgsResidualPlotItem() = default;
...
void QgsResidualPlotItem::paint( QPainter *painter, const QStyleOptionGraphicsItem *itemStyle, QWidget *pWidget )
{
  ...
  {
    const QgsPointXY gcpCoords = e.sourcePoint;
    ...
    QgsLayoutUtils::drawText( painter, QPointF( gcpItemMMX + 2, gcpItemMMY + 2 ), QString::number( e.id ), QFont() );

    mmPixelRatio = maxMMToPixelRatioForEntry( e, gcpItemMMX, gcpItemMMY );
  ...
  for ( const Entry &e : mEntries )
  {
    ...
    const QPointF p2( gcpItemMMX + e.residual.x() * minMMPixelRatio,
                      gcpItemMMY + e.residual.y() * minMMPixelRatio );
...
void QgsResidualPlotItem::setEntries( const QVector<Entry> &entries )
{
  mEntries = entries;
}
...
double QgsResidualPlotItem::maxMMToPixelRatioForEntry( const Entry &p, double pixelXMM, double pixelYMM )
{
  //calculate intersections with upper / lower frame edge depending on the residual y sign
  double upDownDist = std::numeric_limits<double>::max();
  double leftRightDist = std::numeric_limits<double>::max();
...
double QgsResidualPlotItem::angle( QPointF p1, QPointF p2 )
{
  ...
  const double a = std::acos( ( -yDiff * length ) / ( length * length ) ) * 180 / M_PI;
  if ( xDiff < 0 )
    return ( 360 - a );
  return a;
}
