...
QgsImageWarper::QgsImageWarper( QgsFeedback *feedback )
  : mFeedback( feedback )
{}
...
)
{
  ...
  const bool wasCancelled = feedback && feedback->isCanceled();
  return wasCancelled ? QgsImageWarper::Result::Canceled
                      : eErr == CE_None ? QgsImageWarper::Result::Success
                                        : QgsImageWarper::Result::WarpFailure;
}

QgsImageWarper::WarpResult QgsImageWarper::warpFile(
  const QString &input,
  const QString &output,
  const QgsGeorefTransform *georefTransform,
  ResamplingMethod resampling,
...
)
{
  ...
  if ( !georefTransform )
  {
    ...
    result.errorMessage = QStringLiteral( "null transform" );
    return result;
  }
  if ( !georefTransform->parametersInitialized() )
  {
    result.status = WarpStatus::SingularTransform;
    result.errorMessage = QStringLiteral( "transform parameters not initialised (likely singular GCP set)" );
    return result;
  }
  ...
  if ( !probe )
  {
    result.status = WarpStatus::InputUnavailable;
    result.errorMessage = QStringLiteral( "Cannot open source: %1" ).arg( input );
    const char *msg = CPLGetLastErrorMsg();
    if ( msg && *msg )
      result.errorMessage += QStringLiteral( " (%1)" ).arg( QString::fromUtf8( msg ) );
    return result;
  }
  GDALClose( probe );
  ...
  switch ( legacy )
  {
    ...
    case Result::InvalidParameters:
      result.status = WarpStatus::SingularTransform;
      result.errorMessage = QStringLiteral( "invalid warp parameters" );
      break;
    case Result::SourceError:
      result.status = WarpStatus::InputUnavailable;
      result.errorMessage = QStringLiteral( "Cannot open source: %1" ).arg( input );
      break;
    case Result::TransformError:
    case Result::DestinationCreationError:
    case Result::WarpFailure:
    {
      const char *msg = CPLGetLastErrorMsg();
      QString m = QString::fromUtf8( msg ? msg : "unknown GDAL error" );
      if ( m.contains( QStringLiteral( "No space left" ), Qt::CaseInsensitive ) ||
           m.contains( QStringLiteral( "ENOSPC" ), Qt::CaseInsensitive ) )
      {
