...
void SpectralProfileWidget::extractProfile( const QgsPointXY &point, QgsRasterLayer *layer )
{
    ...
    double x = srcPt.x();
    double y = srcPt.y();
    // Guard against degenerate geotransforms (non-georeferenced rasters):
    // dividing by a zero pixel resolution would produce Inf/NaN (#516).
    if ( std::abs( adfGeoTransform[1] ) < 1e-15 || std::abs( adfGeoTransform[5] ) < 1e-15 )
        return;
