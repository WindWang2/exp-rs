        ...
        for ( int b = 0; b < nBands; ++b )
        {
            ...
            for ( int r = box.minR; r <= box.maxR; ++r )
            {
                for ( int c = box.minC; c <= box.maxC; ++c )
                {
                    ...
                    const int level1 = std::clamp( static_cast<int>( ( val1 - minV ) / rangeV * ( nLevels - 1 ) ), 0, nLevels - 1 );
                    ...
                    for ( int d = 0; d < 2; ++d )
                    {
                        ...
                        if ( nr <= box.maxR && nc <= box.maxC && labels[nr * w + nc] == segId )
                        {
                            ...
                            if ( !std::isfinite( val2 ) )
                                continue;
                            const int level2 = std::clamp( static_cast<int>( ( val2 - minV ) / rangeV * ( nLevels - 1 ) ), 0, nLevels - 1 );
