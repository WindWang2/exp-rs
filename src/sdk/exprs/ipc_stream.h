...
/// (passed as void*). The stream TAKES OWNERSHIP of both ends: close()
/// (and the destructor) close them idempotently. Callers must not close
/// the handles themselves after handing them over.
