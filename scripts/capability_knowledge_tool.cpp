...
//
//   gen-meta   Derive the v2 capability sidecars for all rs: operators
...
int cmdGenMeta( const QString &root )
{
  ...
  if ( rsDescriptors.size() != 117 )
  {
    std::cerr << "gen-meta: expected 117 rs: descriptors, found " << rsDescriptors.size()
