// sha256.cpp — compact FIPS 180-4 SHA-256 (see sha256.h for why it is local).
...
inline uint32_t rotr( uint32_t v, int n )
{
  return ( v >> n ) | ( v << ( 32 - n ) );
}
...
std::string sha256Hex( const void *data, std::size_t size )
{
  ...
  // Tail: remaining bytes + 0x80 + length (big-endian, 128-bit).
  uint8_t tail[128] = {};
  const std::size_t rem = size - full * 64;
  std::memcpy( tail, bytes + full * 64, rem );
  tail[rem] = 0x80;
  const std::size_t tailLen = ( rem + 9 <= 64 ) ? 64 : 128;
  ...
  for ( int i = 0; i < 8; ++i )
  {
    for ( int shift = 28; shift >= 0; shift -= 4 )
      out.push_back( kHex[( h[i] >> shift ) & 0xF] );
  }
  return out;
}
