...
class LayerPropertiesDialog(QDialog):
    ...
    def _create_info_tab(self) -> QWidget:
        ...
        crs_text = QTextEdit()
        crs_text.setPlainText(crs_str)
        crs_text.setReadOnly(True)
        crs_text.setMaximumHeight(65)
        crs_text.setStyleSheet("""
            QTextEdit {
        ...
        """)
        c_layout.addRow("Native Projection:", crs_text)
    ...
    def _create_histogram_tab(self) -> QWidget:
        """Creates a tab widget containing a Matplotlib histogram of band values."""
        widget = QWidget()
        layout = QVBoxLayout(widget)
        layout.setContentsMargins(15, 15, 15, 15)
        layout.setSpacing(10)
        
        # 1. Band selector for histogram
        selector_layout = QHBoxLayout()
        selector_layout.addWidget(QLabel("Select Band for Histogram:"))
        
        self.hist_band_combo = QComboBox()
        band_count = self.layer.provider.reader.metadata.get("count", 1)
        self.hist_band_combo.addItems([f"Band {i}" for i in range(1, band_count + 1)])
        selector_layout.addWidget(self.hist_band_combo)
        
        self.compute_hist_btn = QPushButton("Compute Histogram")
        self.compute_hist_btn.setStyleSheet("""
            QPushButton {
                background-color: #1f6feb;
                color: white;
                border: 1px solid #1f6feb;
        ...
        """)
        self.compute_hist_btn.clicked.connect(self._compute_and_plot_histogram)
        selector_layout.addWidget(self.compute_hist_btn)
        
        layout.addLayout(selector_layout)
        
        # 2. Matplotlib Figure Canvas
        self.fig = Figure(figsize=(5, 3), dpi=100)
        self.fig.patch.set_facecolor('#ffffff') # Match dialog background
        self.canvas = FigureCanvas(self.fig)
        self.canvas.setStyleSheet("background-color: #ffffff; border: 1px solid #d4d8de; border-radius: 4px;")
        ...
        self.ax.set_title("Click 'Compute Histogram' to analyze pixel values", fontsize=9, color='#555555', pad=10)
        
        layout.addWidget(self.canvas)
        return widget

    def _compute_and_plot_histogram(self):
        """Downsamples band data and plots pixel distribution and active stretch limits."""
        self.compute_hist_btn.setEnabled(False)
        self.compute_hist_btn.setText("Computing...")
        
        # Process events to show text change
        from PySide6.QtWidgets import QApplication
        QApplication.processEvents()
        
        try:
            band_idx = self.hist_band_combo.currentIndex() + 1
            
            # Read raster band at a downsampled rate (for rapid sub-10ms performance)
            reader = self.layer.provider.reader
            metadata = reader.metadata
            max_dim = max(metadata["width"], metadata["height"])
            scale_factor = max(1, int(max_dim / 150))
            
            band_data = reader.read_raster_band(band_idx, scale_factor)
            
            # Filter non-finite values (inf, nan)
            valid_data = band_data[np.isfinite(band_data)]
            
            if valid_data.size == 0:
                self.ax.clear()
                self.ax.set_title("No valid finite data in this band", fontsize=9, color='red')
                self.canvas.draw()
                return
                
            # Compute histogram
            counts, bins = np.histogram(valid_data, bins=100)
            
            # Clear ax and plot beautifully
            self.ax.clear()
            self.ax.set_facecolor('#ffffff')
            self.ax.spines['top'].set_visible(False)
            self.ax.spines['right'].set_visible(False)
            ...
            self.ax.fill_between(bin_centers, counts, color='#1f6feb', alpha=0.3)
            self.ax.plot(bin_centers, counts, color='#1f6feb', linewidth=1.5)
            self.ax.set_title(f"Pixel Value Distribution - Band {band_idx}", fontsize=9, color='#2f3640', fontweight='bold')
            self.ax.set_xlabel("Pixel Value", fontsize=8, color='#5b6473')
            self.ax.set_ylabel("Frequency", fontsize=8, color='#5b6473')
            
            # Draw vertical dashed lines showing the active contrast stretch min and max limits
            amin, amax = float(valid_data.min()), float(valid_data.max())
            if self.layer.contrast_enhancement != "none":
                if self.layer.min_max_limits_method == "min_max":
                    amin, amax = float(valid_data.min()), float(valid_data.max())
                elif self.layer.min_max_limits_method == "cumulative_cut":
                    amin = float(np.percentile(valid_data, self.layer.cumulative_cut_lower))
                    amax = float(np.percentile(valid_data, self.layer.cumulative_cut_upper))
                elif self.layer.min_max_limits_method == "std_dev":
                    mean = np.mean(valid_data)
                    std = np.std(valid_data)
                    amin = float(mean - self.layer.std_dev_factor * std)
                    amax = float(mean + self.layer.std_dev_factor * std)
                elif self.layer.min_max_limits_method == "user_defined":
                    amin = self.layer.user_min if self.layer.user_min is not None else float(valid_data.min())
                    amax = self.layer.user_max if self.layer.user_max is not None else float(valid_data.max())
            
            # Add vertical lines at amin and amax
            self.ax.axvline(x=amin, color='#d9534f', linestyle='--', linewidth=1.5, label=f"Min Limit ({amin:.2f})")
            self.ax.axvline(x=amax, color='#5cb85c', linestyle='--', linewidth=1.5, label=f"Max Limit ({amax:.2f})")
            ...
            self.ax.clear()
            self.ax.set_title(f"Computation Error: {e}", fontsize=9, color='red')
            self.canvas.draw()
            
        finally:
            self.compute_hist_btn.setEnabled(True)
            self.compute_hist_btn.setText("Compute Histogram")

QgsPropertiesDialog = LayerPropertiesDialog
PropertiesDialog = LayerPropertiesDialog
