...
from PySide6.QtCore import Qt, QRectF, Signal, QSize, QTimer
...
class QgsMapCanvas(QGraphicsView):
    ...
    Rendering is performed synchronously on the main thread to avoid
    SIGSEGV crashes in libproj's DatabaseContext when rasterio.open()
    is called from QThreadPool background threads on CRS-bearing files.
    A deferred refresh via QTimer keeps the UI responsive during rapid
    zoom/pan by coalescing multiple refresh requests.
    """
    ...
    def __init__(self, parent=None):
        ...
        # Deferred refresh timer — coalesces rapid refresh() calls
        self._refresh_timer = QTimer(self)
        self._refresh_timer.setSingleShot(True)
        self._refresh_timer.setInterval(0)  # Fire on next event loop iteration
    ...
    # --- Refresh ---
    def refresh(self):
        # Coalesce rapid refresh requests via deferred timer.
        # This avoids redundant re-renders during rapid scroll/zoom.
        if not self._refresh_timer.isActive():
            self._refresh_timer.start()

    def _do_refresh(self):
        """Execute the actual render synchronously on the main thread."""
        ...
        job.run()  # Synchronous on main thread — safe for PROJ/GDAL

        if hasattr(job, '_last_image') and job._last_image and not job._last_image.isNull():
            image = job._last_image
            # Draw scale bar
    ...
    def _calculate_scale_denominator(self, image_size):
        ...
        ground_width_mm = ext.width() * 1000
    ...
    def renderPreview(self):
        """Backward compat alias — renders synchronously on the main thread."""
        self._do_refresh()
