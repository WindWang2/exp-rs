...
- 扰动地块：显著负斜率（量级 ≤ −3×10⁻⁴ NDVI/天，且明显低于常绿林/水体）；
- 常绿林/水体：接近 0（|斜率| ≤ 2×10⁻⁄）；
- 注意扰动地块的 r² 很低（≈0.05）：线性模型无法同时拟合季节峰与突变，方差被季节项主导——这不是算错，而是模型选择问题（见思考题 2）。

注意**斜率单位是 NDVI/天**，乘 365 才是「年速率」量级——这是最常见的读数错误。

### 8.4 物候参数提取（`rs:temporal_phenology`）

读取 sos/pos/eos/los（DOY/天）波段，对照农田「夏峰」先验解释空间格局（先验：SOS≈DOY 75、POS≈DOY 197、EOS≈DOY 320）。常绿林与水体不构成有效季节：阈值交叉法没有最小振幅门限，这类像元输出的是振幅近 0 的退化值（结合振幅波段判读），不是 NoData。月频采样下物候精度约 ±15 天（原理见上），这是采样密度决定的诚实边界。

### 8.5 异常检测与对照验证（`rs:temporal_anomaly`，运行两次）

以 `baseline_start=2024-03-01`、`baseline_end=2024-08-31`（扰动前的生长季窗口）为基线：

1. 目标时相 `target_time=2024-10-15`（扰动后）：扰动地块应出现连片 z ≤ −2 的负异常（窗口取生长季可避免季节方差稀释 z 值）；
2. 目标时相 `target_time=2024-08-15`（扰动前对照）：不应出现连片**负**异常（对照正值 z≈+1 是季节峰值效应：峰值日本来就高于基线均值，不是异常）。注意全图会散布少量极端负 z——它们来自量化后完全平坦（基线 σ≈0）的像元，z 在 σ→0 处爆炸；这就是判分用中位数与占比、而非均值的原因。

对比两幅异常图：只有前者在扰动地块出现连片负异常，说明检出信号不是方法伪影。
...
| 产物 | 预期 | 判分容差 |
|------|------|----------|
| `nir_summary.tif` | count 波段全图 = 12（12 期全部有效） | 意图 T7 |
| `ndvi_series.tif` | 12 波段；常绿林 0.65–0.85，水体 −0.20–0.00，农田夏冬对比明显 | 意图 T1/T2 |
| `ndvi_trend.tif` | 扰动地块 slope ≤ −3×10⁻⁴ 且显著低于林地/水体（斜率差 ≥ 2×10⁻⁴）；不设 r² 达标线（季节方差主导，r²≈0.05 是模型选择考点） | 意图 T3（斜率按天） |
| `phenology.tif` | 农田 SOS 70–170、POS 170–230、EOS 230–330 DOY，LOS 100–250 天；林地/水体振幅 < 0.15（退化值） | 意图 T4 |
| `anomaly_2024-10-15.tif` | 扰动地块 anomaly 均值 ≤ −2.0（基线 2024-03-01..2024-08-31） | 意图 T5 |
| `anomaly_2024-08-15_control.tif` | 稳健判据：地块中位数 ≥ −1.0；z ≤ −2 占比：地块 ≤ 1%、全图 ≤ 5% | 意图 T6 |
...
5. 水体 NDVI 为负、常绿林振幅近 0，二者的物候输出都是振幅近 0 的退化值——它们的机理一样吗？如何区分？
