...
传感器 DN 与后向散射的换算：**σ0 = (DN/A)² − noise**（本实验 noise=0，A=100）。两景必须用同一 A 定标，否则幅度系统差会被误判为变化。变化检测在 dB 域做对数比：**Δ = 10·log10(σ0_B/σ0_A)**——乘性斑在对数域近似加性高斯，比值对称（±Δ 分别对应增强/减弱）。Otsu 法在 Δ 直方图上自动求类间方差最大的阈值。
