...
| 2 | Corrupt governance SQLite (corrupt while store open) | save probes integrity (throttled: a positive `PRAGMA quick_check` is trusted for 2 s) and falls back to the cached document with a `workspace.stale_document_repersisted` warning; detection is best-effort — SQLite's page cache can serve a small corrupted DB, in which case the session keeps serializing cached-page content; store writes fail with typed `store.*` diagnostics | `storeIntegrityOk` contract; `test_workspace_project_v3` "Save with an unusable store falls back to the cached document loudly" pins the warning branch |
...
| 4 | Hot WAL at snapshot time | `wal_checkpoint(TRUNCATE)` stepped (busy row inspected — exec alone would hide a blocked checkpoint); snapshot refuses with a checkpoint error unless the reset completed; only the main DB file is copied (read-only stores cannot snapshot) | `test_workspace_services` "Snapshot preserves governed writes that live only in the WAL"; `GovernanceStore::checkpointForBackup` |
| 5 | Concurrent writer during snapshot | in-process writers serialize on the store mutex (snapshot succeeds integrity-checked or refuses naming the checkpoint); a SECOND-process writer between checkpoint and copy is a documented residual window | `test_workspace_services` "Snapshot under a concurrent writer stays openable" |
...
| 17 | Legacy checkpoint (pre-identity schema) | fail-conservative: unverifiable completed steps re-execute | ADR 0130 decision 7; gate enforces `outputSizeBytes > 0` |
...
| 24 | Disk full (governance store) | every write path checks BEGIN, step and COMMIT, rolls back, and reports typed failures; anchor failures additionally land in the audit log (`run.anchor_failed`) | covered-by-contract (rows 6–7 mechanism); portable fault injection not available — accepted debt, recorded in `.planning/data-runtime-governance-4/REVIEW_LOG.md` round 1 |
