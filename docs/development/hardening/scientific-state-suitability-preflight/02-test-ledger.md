...
|---|---|---|---|---|
| 1 | `sicnu_preflight` + schema test outside build graph (grep: no `add_subdirectory(src/preflight)`; no registration of `test_preflight_report_schema`) | root CMakeLists wires the Qt-free leaf; tests/CMakeLists registers the schema suite in the sdk lane (`Sicnu::Preflight` link, mirrors `test_scientific_state_core`) | the 7 schema cases (fail-closed readers, canonical bytes, tamper digest) never compiled or ran | CI: `test_preflight_report_schema` target exists and passes |
