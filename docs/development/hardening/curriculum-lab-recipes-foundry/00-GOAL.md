...
|----|----------|--------|-----|
| S1 | P0 build/CI | `src/recipes` never compiled (no CMake target); `sicnu_lab_runtime` never linked; 3428 lines of teaching tests (`test_lab_runtime`, `test_curriculum`, `test_recipe_*`, …) registered nowhere — the whole foundation was untested from CI's point of view | **dedup**: master merged the same wiring while this branch worked (#1246, with a wiring-drift oracle); this branch rebased onto it, dropped its duplicate registrations, and keeps only the new `test_teaching_foundation_e2e` wiring. All defect fixes below remain novel — master's advance touched none of these files |
