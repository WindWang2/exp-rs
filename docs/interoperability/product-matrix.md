...
|---|---|---|---|---|---|---|---|---|
| Landsat Collection 1/2 (MTL) | ● | platform/sensor/level/date/radiometric keys | ● (MTL-declared FILE_NAME_* inventory) | ● (OLI/TIRS + legacy TM/ETM mappings, thermal ST_*, QA masks) | ● | ● (QA_PIXEL/QA_RADSAT/MSK_*) | ● (Partial/UnsupportedVersion for unknown COLLECTION_NUMBER) | scenes without MTL fall back to generic raster |
| Sentinel-2 SAFE (L1C/L2A) | ● | platform/level/cloud cover (MTD) | ● (granule-bounded, R10m/R20m/R60m groups distinct) | ● (B01–B12/B8A/SCL/AOT/TCI roles + wavelengths) | ● | ● (SCL/MSK as mask assets) | ● (manifest/measurements/core-band B02 checks) | multi-granule enumeration bounded at 16 |
...
| MODIS (HDF4/HDF5 container) | ● | platform/product level (declared) | ● (GDAL subdatasets, bounded at 64) | ◐ (subdataset naming; roles via MODIS band tables for known products) | ◐ (tile indices h/v from names) | ◐ | ● (unopenable container → Invalid; zero subdatasets → PartialReadable) | sinusoidal georeferencing flows live in the processing import path |
