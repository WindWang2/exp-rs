...
//   1. coverage: exactly 115/115 rs: operators carry a v2 capability sidecar
...
TEST_CASE( "D8 coverage: 115/115 rs operators carry capability metadata",
           "[capability][d8][coverage]" )
{
    ...
    REQUIRE( registry.size() == 115 );
    REQUIRE( boot.catalog.loadProblems().empty() );
    REQUIRE( catalog.size() == 115 );
           ...
           "[capability][d8][mirror]" )
{
    ...
    for ( const std::string &id : boot.catalog.entryIds() )
    {
        ...
        if ( mirror.isNull() )
          continue; // mirror covers the preflight subset; D8 covers all 115
        const Json::Value block = boot.catalog.capability( id );
