...
TEST_CASE( "SessionState: workflow snapshot round-trip", "[classify][session]" )
{
  ...
  RsClassifySessionState::Snapshot in;
  ...
  in.lastModelPath = QStringLiteral( "/tmp/model.xml" );
  in.classifierKind = 1;
  in.trainRatio = 0.85;
  in.wandTolerance = 12.5;

  RsClassifySessionState s;
  s.saveSnapshot( in );
  s.setLastRoisPath( in.lastRoisPath );

  const auto out = s.restoreSnapshot();
  ...
  REQUIRE( out.lastModelPath == QStringLiteral( "/tmp/model.xml" ) );
  REQUIRE( out.classifierKind == 1 );
  REQUIRE( out.trainRatio == Catch::Approx( 0.85 ) );
  REQUIRE( out.wandTolerance == Catch::Approx( 12.5 ) );
