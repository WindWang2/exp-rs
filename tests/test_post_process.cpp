...
TEST_CASE( "PostProcess: majority 3x3 tie preserves center pixel", "[classify][post]" )
{
  ...
  cv::Mat dst;
  REQUIRE( RsPostProcess::majorityFilter( src, dst, 3, nullptr ) );
  // Center pixel is 2, tied with class 1 (4 vs 4). Center must stay 2!
  REQUIRE( dst.at<int>( 1, 1 ) == 2 );
}
