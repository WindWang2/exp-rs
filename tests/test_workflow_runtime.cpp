...
TEST_CASE( "Builtin workflows registered", "[workflow]" )
{
  ...
  REQUIRE( rt.hasDefinition( "tool.rs.apply_mask" ) );
  // Workspace labs
...
TEST_CASE( "Preprocessing DAG chains operators with artifact flow", "[workflow][preprocess]" )
{
  ...
  CHECK( d->steps[3].operatorId == "rs:apply_mask" );
  CHECK( d->steps[4].operatorId == "rs:spectral_index" );
  ...
  // The QA-mask side branch is applied to the corrected product (ADR 0088) so
  // the final index computation sees analysis-ready, cloud-free pixels.
  CHECK( d->steps[3].params["input"].asString() == "$atmospheric.corrected" );
  CHECK( d->steps[3].params["mask"].asString() == "$qa_mask.mask" );
  CHECK( d->steps[4].params["input"].asString() == "$apply_mask.analysis_ready" );
