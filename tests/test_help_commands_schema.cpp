...
QJsonDocument parseBytes( const QByteArray &bytes, QString *error )
{
  QJsonParseError pe;
...
QJsonDocument loadCommands( QString *error )
{
...
{
  QString err;
  const QJsonDocument doc = loadCommands( &err );
  REQUIRE( err.isEmpty() );
  REQUIRE( doc.isArray() );
           ...
           "[help][commands][schema][eol]" )
{
  ...
  QString err1, err2;
  const QJsonDocument asIs = parseBytes( bytes, &err1 );
  REQUIRE( err1.isEmpty() );

  // Simulate a Windows checkout: rewrite every LF to CRLF, then parse.
  QByteArray crlf = bytes;
  crlf.replace( "\n", "\r\n" );
  const QJsonDocument asCrlf = parseBytes( crlf, &err2 );
  REQUIRE( err2.isEmpty() );
