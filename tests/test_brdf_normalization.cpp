...
//  * Published special values: k_vol = 0 and k_geo = 0 exactly at nadir sun
//    + nadir view (Lucht et al. 2000 kernel definitions — this is why f_iso
//    is the nadir BRF in the MODIS literature); both kernels are reciprocal
//    (symmetric under sun↔view swap); the geometric kernel is positive at
//    the hotspot and negative at large phase angles.
//  * Scripted/hand reference values: k_vol(30°,0°,·) = −0.03151 and the
//    k_geo references below were computed with an independent scripted
//    implementation of the published formulas (outside this codebase's
//    production code), plus in-comment hand derivations.
//  * An exact identity anchor for the c-factor: the leveling factor is
//    DEFINED as c = mean(y)/mean(x), so mean(c·x) = mean(y) must hold
//    exactly, and for pairs generated as y = k·x it must give c = k.
//
...
{
    // Both kernels vanish at nadir sun + nadir view (MODIS convention: f_iso
    // is the nadir BRF).
    CHECK( rossThick( 0.0, 0.0, 0.0 ) == Catch::Approx( 0.0 ).margin( 1e-12 ) );
    CHECK( liSparseReciprocal( 0.0, 0.0, 0.0 ) == Catch::Approx( 0.0 ).margin( 1e-12 ) );
    // The geometric kernel peaks positively at the hotspot (shadowing theory)
    // and is negative at large phase angles.
    CHECK( liSparseReciprocal( 45.0, 45.0, 0.0 ) > 0.0 );
    CHECK( liSparseReciprocal( 45.0, 45.0, 180.0 ) < 0.0 );
}
...
TEST_CASE( "hand-computed kernel reference values", "[brdf]" )
{
    ...
    // k_vol(40°, 30°, 0°): independent reference (scripted implementation of
    //   the published formula, computed outside the production code):
    //   cos ξ = 0.98481, ξ = 0.17453 → 0.16352
    CHECK( rossThick( 40.0, 30.0, 0.0 ) == Catch::Approx( 0.16352 ).margin( 1e-4 ) );

    // k_geo(30°, 0°, ·): D = tan30° = 0.57735; cos t = 2·0.57735/2.15470 =
    //   0.53583, t = 1.00685, O = (1.00685 − 0.45273)·2.15470/π = 0.38007;
    //   cos ξ = cos30° = 0.86603 → last term 0.5·1.86603·1.15470 = 1.07736;
    //   k_geo = 0.38007 − 1.15470 − 1 + 1.07736 = −0.69822 (independent
    //   scripted reference −0.698222)
    CHECK( liSparseReciprocal( 30.0, 0.0, 0.0 ) == Catch::Approx( -0.69822 ).margin( 1e-4 ) );

    // k_geo(45°, 45°, 0°) at the hotspot: cos ξ = 1, D = 0, cos t = 0 →
    //   t = π/2, O = sec45° = 1.41421, last term 2·sec45°² /2·(1+1)·... =
    //   2.0; k_geo = 1.41421 − 1.41421 − 1.41421 + 2.0 = 0.58579 (independent
    //   scripted reference 0.585786 — positive at the hotspot, as shadowing
    //   physics requires).
    CHECK( liSparseReciprocal( 45.0, 45.0, 0.0 ) == Catch::Approx( 0.58579 ).margin( 1e-4 ) );
...
TEST_CASE( "anisotropy factor refuses nonphysical geometry and weights", "[brdf]" )
{
    ...
    // Weights that flip the factor nonphysical must refuse: at (60°,60°,180°)
    // k_vol ≈ 0.612, k_geo = −3 exactly (cos ξ = −0.5, t = 0), so
    // f = 1 + 0·0.612 + 10·(−3) < 0.
    CHECK_FALSE( anisotropyFactor( 60.0, 60.0, 180.0, 0.0, 10.0, &f, &err ) );
    CHECK_FALSE( err.isEmpty() );
    CHECK( anisotropyFactor( 60.0, 60.0, 180.0, 100.0, -1000.0, &f, &err ) );
    CHECK( f > 0.0 ); // those weights keep the factor positive (k_geo < 0 helps)
}
...
TEST_CASE( "c-factor equals the mean ratio and preserves means exactly", "[brdf]" )
{
    // Pairs generated as y = k·x (pure gain difference): the leveling factor
    // must return exactly c = k, and mean(c·x) = mean(y) holds by definition.
    const double k = 1.25;
    PairStatistics reg;
    double x = 0.05;
    for ( int i = 0; i < 200; ++i )
    ...
    CHECK( c == Catch::Approx( k ).margin( 1e-12 ) );

    // Identity oracle: mean(c·x) equals mean(y), recomputed independently.
    ...
    CHECK( c * ( sx / 200.0 ) == Catch::Approx( sy / 200.0 ).margin( 1e-12 ) );

    // Application: rho2' = c·rho2, NaN passes through.
    CHECK( applyPairNormalization( 0.4f, c ) == Catch::Approx( 0.4f * k ).epsilon( 1e-6 ) );
    CHECK( std::isnan(
        applyPairNormalization( std::numeric_limits<float>::quiet_NaN(), c ) ) );
}

TEST_CASE( "c-factor refuses weak samples and unusable statistics", "[brdf]" )
{
    double c = 0.0;
    QString err;

    PairStatistics tooFew;
    tooFew.add( 0.1, 0.12 );
    tooFew.add( 0.2, 0.21 );
    CHECK_FALSE( tooFew.fitCFactor( 30, &c, &err ) );
    CHECK( err.contains( QLatin1String( "pairs" ) ) );
    // The absolute floor is 2 pairs: with minPairs = 1 the fit succeeds.
    CHECK( tooFew.fitCFactor( 1, &c ) );
    CHECK( c == Catch::Approx( ( 0.12 + 0.21 ) / ( 0.1 + 0.2 ) ).margin( 1e-12 ) );

    // Non-positive samples are never usable pairs (signs cannot be leveled
    // multiplicatively).
    PairStatistics withJunk;
    for ( int i = 0; i < 40; ++i )
    ...
    // x: 0.1..0.49 (mean 0.295), y: 0.11..0.50 (mean 0.305) → c = 61/59.
    CHECK( c == Catch::Approx( 0.305 / 0.295 ).margin( 1e-12 ) );
    CHECK( c > 0.0 );

    // A target mean that is non-positive (impossible via add()'s domain
    // guard, but the guard keeps the accumulator internally consistent).
    PairStatistics empty;
    CHECK_FALSE( empty.fitCFactor( 30, &c, &err ) );
}
