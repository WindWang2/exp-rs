# GOAL — F19 · Deployment, Offline Packaging & Runtime Diagnostics 11.0

> 本文件为 `/goal` 指令原文逐字存档（required by goal-template runbook）。

/goal  target-agent=zcode  model=GLM-5.3-flash  budget=500000000  subagents<=2  ci=none  autonomy=full  defaults=best

> **Mission:** Windows/Linux离线部署、依赖审计、可重复打包与启动诊断
> **Target model:** GLM-5.3-flash（长跑大预算；以严格 Oracle/ledger 防止低质量漂移）
> **Branch:** `zcode/deployment-packaging-11`
> **Worktree:** `../exp-rs-deployment-packaging-11`
...
现有 Windows offline lab profile、AppImage、offline bundle 已具雏形，但跨平台依赖体量巨大，值得把可重复构建、capability manifest、驱动/Qt/GDAL诊断、包完整性和现场故障定位建设成产品级能力。
...
建立 `.planning/deployment-packaging-11/PARALLEL_OWNERSHIP.md`：列出所有 open PR / remote branch 的 changed-file ownership、与本 track 的文件级交集、处理策略。规则：
...
最终产品不是"写了一批代码"，而是：**Windows/Linux离线部署、依赖审计、可重复打包与启动诊断**，并且其 authority、failure semantics、resource bounds、provenance、tests、docs、agent/CLI/GUI surface（适用时）相互一致。先证明已有能力和真实缺口，再实现；对所有科学公式/坐标/单位/时间/NoData 语义采用独立真值，不允许测试复用被测实现来制造绿灯。

## Work packages

| ID | Package | Required deliverables |
|---|---|---|
| A | dependency inventory | Qt/GDAL/PROJ/GEOS/QGIS/OTB/ITK/model providers/driver plugins 的版本和动态库闭包。 |
| B | deterministic bundle manifest | 逐文件 digest、来源/license、build options、capabilities、schema version。 |
| C | Windows offline | VS/vcpkg/Qt detection、portable paths、DLL/plugin search、中文路径、lab profile。 |
| D | Linux packaging | AppImage/portable tree、RPATH/plugin path、GPU optional libs、headless CLI。 |
| E | first-run diagnostics | 本地自检 Qt platform/GDAL drivers/PROJ db/SSL/optional providers/permissions/temp/scratch。 |
| F | offline guarantee | network deny、remote feature typed refusal、loopback tests、no silent update check。 |
| G | upgrade/migration | settings/schema/cache compatibility、side-by-side install、rollback guidance，不做自动联网更新。 |
| H | clean-machine simulation | container/isolated env scripts、bundle verify、smoke app/CLI/MCP、size budgets。 |
...
1. bundle manifest 可在另一目录验证且 tamper 必失败
2. offline 模式启动/核心实验零网络
3. 缺 DLL/plugin/provider 时诊断指向具体缺项
4. packaging smoke 两次通过或明确 not-executed host limitation
...
```bash
git worktree add ../exp-rs-deployment-packaging-11 -b zcode/deployment-packaging-11 origin/master
cd ../exp-rs-deployment-packaging-11
```

1. 立即把本 GOAL 原文保存到 `.planning/deployment-packaging-11/GOAL.md`，创建 `PLAN.md`、`BASELINE.md`、`DECISIONS.md`、`EVIDENCE.md`、`REVIEW_LOG.md`、`PR_BODY.md`、`PARALLEL_OWNERSHIP.md`、`TEST_MATRIX.md`。
2. 按 repo 现有模式让 `.planning/deployment-packaging-11/` 可被 git 跟踪；用 `git check-ignore -v` 验证。不要大改 `.gitignore`。
...
8. `git push -u origin zcode/deployment-packaging-11`；禁止 force push。
9. 创建独立 PR：`gh pr create --base master --head zcode/deployment-packaging-11 --body-file .planning/deployment-packaging-11/PR_BODY.md ...`。不要 merge，不等待线上 checks。
