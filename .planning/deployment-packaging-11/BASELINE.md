...
| `scripts/windows/*` | `_env.cmd` (vswhere→vcvars, tool probes, hard bounds `-j2`, offscreen; **one hardwired user-profile default `C:\Users\<redacted-user>\...vcpkg_installed`**), setup.cmd, check_mcp.cmd(+ps1, MCP stdio round-trip), run_lab.cmd, grade_all.cmd | complete |
