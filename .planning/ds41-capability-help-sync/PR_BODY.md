...
- Windows autocrlf robustness: the D8 page-diff gate and the #707 sidecar byte gate compared disk content against LF output; on Windows checkouts (`core.autocrlf=true`, no `.gitattributes` rule for `*.md`/`data/**`) a CORRECT tree went red. Both comparisons now normalize CRLF first (content contract unchanged); proven by converting the corpus to CRLF and re-running all four gates green.
