# /goal — F07 · Linked Visual Analytics & Multi-View GIS 11.0

/goal  target-agent=zcode  model=GLM-5.3-flash  budget=500000000  subagents<=2  ci=none  autonomy=full  defaults=best

> **Mission:** 地图/图表/多视图选择、光标、范围与图层可见性联动
> **Target model:** GLM-5.3-flash（长跑大预算；以严格 Oracle/ledger 防止低质量漂移）
> **Branch:** `zcode/linked-visual-analytics-11`
> **Worktree:** `../exp-rs-linked-visual-analytics-11`
...
Workbench 10.0 已交付 VaDataSource/VaChartWidget/ViewLinkController，但明确指出 ViewLinkController 未装配 shell、cursor/visibility 未同步、进程级 VA selection hub 尚缺。
...
建立 `.planning/linked-visual-analytics-11/PARALLEL_OWNERSHIP.md`：列出所有 open PR / remote branch 的 changed-file ownership、与本 track 的文件级交集、处理策略。规则：
...
**默认 read-only / 避免并发冲突：**
- src/app/workbench/mission_context* and main_window_* while #991 open
- D19 dataset UI
...
最终产品不是"写了一批代码"，而是：**地图/图表/多视图选择、光标、范围与图层可见性联动**，并且其 authority、failure semantics、resource bounds、provenance、tests、docs、agent/CLI/GUI surface（适用时）相互一致。先证明已有能力和真实缺口，再实现；对所有科学公式/坐标/单位/时间/NoData 语义采用独立真值，不允许测试复用被测实现来制造绿灯。

## Work packages

| ID | Package | Required deliverables |
|---|---|---|
| A | SelectionHub authority | typed selection event、source/generation、object/layer/feature/pixel/region identities、loop suppression。 |
| B | N-view extent link | 跨 CRS、per-view group、throttle、history/undo、detach lifecycle。 |
| C | cursor/pixel link | 地图↔地图↔图表 hover/crosshair，采样异步化与 stale-generation 丢弃。 |
| D | layer visibility/style link | 按 stable layer identity 选择性同步 visibility/opacity/time/frame，不用名字猜。 |
| E | brushing & filtering | hist/scatter/profile/table/map 间双向 brushing，bounded payload 与大样本抽样诚实标记。 |
| F | view.* command family | 通过 command registry 暴露 group/link/unlink/focus/sync 状态；若 #991 未合入不碰 main_window。 |
| G | lifecycle/concurrency | QPointer、窗口关闭、图层删除、project reset、后台 scan cancel。 |
| H | offscreen UI tests | 循环抑制、CRS、100k logical selection、快速窗口销毁、command availability。 |
...
1. 任何联动事件不会形成无限反馈环
2. 关闭窗口/删图层后无 UAF/stale callback
3. 跨 CRS extent/cursor 有 known-answer
4. visual/view tests 两次通过
...
```bash
git worktree add ../exp-rs-linked-visual-analytics-11 -b zcode/linked-visual-analytics-11 origin/master
cd ../exp-rs-linked-visual-analytics-11
```

1. 立即把本 GOAL 原文保存到 `.planning/linked-visual-analytics-11/GOAL.md`，创建 `PLAN.md`、`BASELINE.md`、`DECISIONS.md`、`EVIDENCE.md`、`REVIEW_LOG.md`、`PR_BODY.md`、`PARALLEL_OWNERSHIP.md`、`TEST_MATRIX.md`。
2. 按 repo 现有模式让 `.planning/linked-visual-analytics-11/` 可被 git 跟踪；用 `git check-ignore -v` 验证。不要大改 `.gitignore`。
...
8. `git push -u origin zcode/linked-visual-analytics-11`；禁止 force push。
9. 创建独立 PR：`gh pr create --base master --head zcode/linked-visual-analytics-11 --body-file .planning/linked-visual-analytics-11/PR_BODY.md ...`。不要 merge，不等待线上 checks。
