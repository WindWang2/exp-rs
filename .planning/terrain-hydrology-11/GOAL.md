...
README 所示 terrain 目前以 slope/aspect/hillshade/roughness/TRI/TPI 为主，仍有空间建设水文、viewshed/horizon、地貌分类、scale-space terrain 和大 DEM 有界算法。
...
建立 `.planning/terrain-hydrology-11/PARALLEL_OWNERSHIP.md`：列出所有 open PR / remote branch 的 changed-file ownership、与本 track 的文件级交集、处理策略。规则：
...
- `src/processing/algorithms/*terrain*`
- `src/operators/rs/*terrain*`
- `src/app/*terrain*`
- `tests/*terrain*`
...
**默认 read-only / 避免并发冲突：**
- D18/D19
- radiometric correction files owned by F14 except shared terrain products
...
最终产品不是"写了一批代码"，而是：**DEM水文、可视域、太阳地形与流式地形分析平台**，并且其 authority、failure semantics、resource bounds、provenance、tests、docs、agent/CLI/GUI surface（适用时）相互一致。先证明已有能力和真实缺口，再实现；对所有科学公式/坐标/单位/时间/NoData 语义采用独立真值，不允许测试复用被测实现来制造绿灯。

## Work packages

| ID | Package | Required deliverables |
|---|---|---|
| A | DEM conditioning | NoData、sink fill/breach、flat resolution、unit/vertical datum warnings。 |
| B | flow direction/accumulation | D8/D∞ 可选、边界、流域出口、tile/external-memory design。 |
| C | watershed/stream | threshold、stream order、basin polygons、connectivity、矢量化。 |
| D | viewshed/horizon | observer geometry、curvature/refraction 可选、multi-observer、遮挡。 |
| E | solar terrain | hillshade series、horizon/shadow duration，为 F14 暴露稳定产品而不耦合。 |
| F | landform/multiscale | TPI scales、curvature、geomorphon/terrain position 类方法与参数解释。 |
| G | UI/agent surface | DEM inspect、profile、watershed/observer交互，通过独立面板。 |
| H | synthetic DEM truth | plane/cone/pit/ridge/channel，closed-form 与 edge/NoData/large grid tests。 |
...
1. 水文 mass/connectivity 不变量有 synthetic truth
2. viewshed 在简单平面/脊线真值正确
3. 大 DEM 路径有明确内存上限与取消
4. terrain suites 两次通过
...
```bash
git worktree add ../exp-rs-terrain-hydrology-11 -b zcode/terrain-hydrology-11 origin/master
cd ../exp-rs-terrain-hydrology-11
```

1. 立即把本 GOAL 原文保存到 `.planning/terrain-hydrology-11/GOAL.md`，创建 `PLAN.md`、`BASELINE.md`、`DECISIONS.md`、`EVIDENCE.md`、`REVIEW_LOG.md`、`PR_BODY.md`、`PARALLEL_OWNERSHIP.md`、`TEST_MATRIX.md`。
2. 按 repo 现有模式让 `.planning/terrain-hydrology-11/` 可被 git 跟踪；用 `git check-ignore -v` 验证。不要大改 `.gitignore`。
...
8. `git push -u origin zcode/terrain-hydrology-11`；禁止 force push。
9. 创建独立 PR：`gh pr create --base master --head zcode/terrain-hydrology-11 --body-file .planning/terrain-hydrology-11/PR_BODY.md ...`。不要 merge，不等待线上 checks。
