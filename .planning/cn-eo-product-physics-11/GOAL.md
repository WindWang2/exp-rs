# GOAL — F02 · Chinese EO Product Physics & Import Platform 11.0

/goal  target-agent=zcode  model=GLM-5.3-flash  budget=500000000  subagents<=2  ci=none  autonomy=full  defaults=best

> **Mission:** 扩展国产卫星产品家族、传感器真值、校准/RPC与稳健导入
> **Target model:** GLM-5.3-flash（长跑大预算；以严格 Oracle/ledger 防止低质量漂移）
> **Branch:** `zcode/cn-eo-product-physics-11`
> **Worktree:** `../exp-rs-cn-eo-product-physics-11`
...
Product Physics 10.0 已统一 registry/CRESDA import，并明确 GF-3、GF-4、GF-5/AHSI、ZY-1 02B/02D/02E、CBERS 等仍是 recognized-but-not-adapted，同时 registry 类型硬化、真实包兼容与稳健导入仍有债务。
...
对所有 open PR（至少含启动时仍开放的 #991/#992）继续读取：
...
建立 `.planning/cn-eo-product-physics-11/PARALLEL_OWNERSHIP.md`：列出所有 open PR / remote branch 的 changed-file ownership、与本 track 的文件级交集、处理策略。规则：
...
- `src/geospatial/products/**`
- `data/products/**`
- `src/operators/io/*product*`
- `src/app/*product*`
- `tests/*cn_product*`
- `docs/products/**`
...
**默认 read-only / 避免并发冲突：**
- D18/D19 owned paths
- SAR algorithm kernels outside product decode
...
最终产品不是"写了一批代码"，而是：**扩展国产卫星产品家族、传感器真值、校准/RPC与稳健导入**，并且其 authority、failure semantics、resource bounds、provenance、tests、docs、agent/CLI/GUI surface（适用时）相互一致。先证明已有能力和真实缺口，再实现；对所有科学公式/坐标/单位/时间/NoData 语义采用独立真值，不允许测试复用被测实现来制造绿灯。

## Work packages

| ID | Package | Required deliverables |
|---|---|---|
| A | registry schema 2.0 | 严格字段类型、单位、可选性、generation/version、unknown key forward-compat；生成 validator 与 drift tests。 |
| B | GF-3 SAR / GF-4 | 依据公开/仓库可验证样例建立产品识别、sidecar generation、band/polarization、RPC/orbit/calibration；未知格式拒绝猜测。 |
| C | GF-5/AHSI | 高光谱 band axis、wavelength/FWHM、bad-band/calibration、HDF/GDAL subdataset inventory 和 stack/virtual access。 |
| D | ZY-1 02B/02D/02E | PMS/HSI 等实际模式按可验证元数据适配，处理 sibling constituents、RPC、band roles。 |
| E | CBERS/HJ 扩展 | 区分 CRESDA/INPE 等 generation，不混用 schema；完善 HJ rpc_rpb 与 sidecar diagnostics。 |
| F | 导入规划与错误诊断 | ImportPlan dry-run、constituent graph、partial-readable、checksum、read-only source、中文路径、取消/原子输出。 |
| G | GUI/CLI/Agent parity | 产品预检、缺失项可视化、schema/结果 parity；不在 UI 复制产品逻辑。 |
| H | fixture corpus | 小型合法/损坏/缺字段/多 generation fixtures + golden metadata；真实大影像不入库。 |
...
1. 所有新增 family 的支持状态来自 registry 且 unknown/unsupported 不静默降级
2. 校准/波长/RPC 等物理量有来源、单位和 provenance；无证据不填默认
3. 导入失败零半成品且 read-only/中文路径/取消测试通过
4. product suites 两次全绿并通过 schema drift review
...
```bash
git worktree add ../exp-rs-cn-eo-product-physics-11 -b zcode/cn-eo-product-physics-11 origin/master
cd ../exp-rs-cn-eo-product-physics-11
```

1. 立即把本 GOAL 原文保存到 `.planning/cn-eo-product-physics-11/GOAL.md`，创建 `PLAN.md`、`BASELINE.md`、`DECISIONS.md`、`EVIDENCE.md`、`REVIEW_LOG.md`、`PR_BODY.md`、`PARALLEL_OWNERSHIP.md`、`TEST_MATRIX.md`。
2. 按 repo 现有模式让 `.planning/cn-eo-product-physics-11/` 可被 git 跟踪；用 `git check-ignore -v` 验证。不要大改 `.gitignore`。
...
8. `git push -u origin zcode/cn-eo-product-physics-11`；禁止 force push。
9. 创建独立 PR：`gh pr create --base master --head zcode/cn-eo-product-physics-11 --body-file .planning/cn-eo-product-physics-11/PR_BODY.md ...`。不要 merge，不等待线上 checks。
