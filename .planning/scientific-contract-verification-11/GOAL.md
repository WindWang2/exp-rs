# /goal — F09 · Scientific Contract & Verification Platform 11.0

/goal  target-agent=zcode  model=GLM-5.3-flash  budget=500000000  subagents<=2  ci=none  autonomy=full  defaults=best

> **Mission:** 全算子确定性/数值/NoData/取消/provenance契约和高可信验证体系  
> **Target model:** GLM-5.3-flash（长跑大预算；以严格 Oracle/ledger 防止低质量漂移）  
> **Branch:** `zcode/scientific-contract-verification-11`  
> **Worktree:** `../exp-rs-scientific-contract-verification-11`  
...
Verification 10.0 建立 scientific_contract registry 和 drift gates，但明确还有大量 determinismGrade stamping 债务与 L3–L7 证据覆盖不足。
...
建立 `.planning/scientific-contract-verification-11/PARALLEL_OWNERSHIP.md`：列出所有 open PR / remote branch 的 changed-file ownership、与本 track 的文件级交集、处理策略。规则：
...
**默认 read-only / 避免并发冲突：**
- D18/D19 implementation paths except read-only contract inspection
...
最终产品不是“写了一批代码”，而是：**全算子确定性/数值/NoData/取消/provenance契约和高可信验证体系**，并且其 authority、failure semantics、resource bounds、provenance、tests、docs、agent/CLI/GUI surface（适用时）相互一致。先证明已有能力和真实缺口，再实现；对所有科学公式/坐标/单位/时间/NoData 语义采用独立真值，不允许测试复用被测实现来制造绿灯。

## Work packages

| ID | Package | Required deliverables |
|---|---|---|
| A | contract census 2.0 | 对所有一等 rs/io/model/cartography operator 建立 live census，去重 authority，找缺 stamp/冲突声明。 |
| B | determinism truth | 逐算子验证 bit_exact/tolerance/stochastic/seeded，修正 stamp 与 sidecar，禁止批量拍脑袋。 |
| C | metamorphic oracle | 比例/平移/旋转/波段重排/NoData/CRS/time 等领域不变量，按适用族生成测试。 |
| D | numeric reference lane | 对重点算法建立独立 closed-form/高精度 reference，不复用实现函数当 oracle。 |
| E | failure/cancel/atomic lane | 错误码、取消时延、半成品、磁盘满、OOM、corrupt input。 |
| F | cross-surface drift | descriptor↔sidecar↔help↔agent↔CLI↔LabSpec↔contract graph 双向一致。 |
| G | verification ladder 11 | L0–L7 capability-aware，本机缺依赖明确 skipped/not-built reason，不引用在线 CI。 |
| H | readiness report | 机器可读 evidence index、baseline delta、pre-existing failure 分类。 |
...
1. live registry 中每个 first-party operator 均有唯一 scientific contract 或明确 exemption
2. determinism claim 有执行证据，不存在“sidecar 自说自话”
3. metamorphic/reference tests 能抓到至少一批人为注入的 mutation samples
4. verification targeted lanes 连续两次通过
...
```bash
git worktree add ../exp-rs-scientific-contract-verification-11 -b zcode/scientific-contract-verification-11 origin/master
cd ../exp-rs-scientific-contract-verification-11
```

1. 立即把本 GOAL 原文保存到 `.planning/scientific-contract-verification-11/GOAL.md`，创建 `PLAN.md`、`BASELINE.md`、`DECISIONS.md`、`EVIDENCE.md`、`REVIEW_LOG.md`、`PR_BODY.md`、`PARALLEL_OWNERSHIP.md`、`TEST_MATRIX.md`。
2. 按 repo 现有模式让 `.planning/scientific-contract-verification-11/` 可被 git 跟踪；用 `git check-ignore -v` 验证。不要大改 `.gitignore`。
...
8. `git push -u origin zcode/scientific-contract-verification-11`；禁止 force push。
9. 创建独立 PR：`gh pr create --base master --head zcode/scientific-contract-verification-11 --body-file .planning/scientific-contract-verification-11/PR_BODY.md ...`。不要 merge，不等待线上 checks。
