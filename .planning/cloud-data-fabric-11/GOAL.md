...
Data Fabric 10.0 已有 catalog/virtual cube/chunk plan/prefetch/mirror，但明确留下 s3/gs/az range-cache key、远程离线 mirror replay、multidim descriptor slicing 和更完整 execution telemetry。
...
建立 `.planning/cloud-data-fabric-11/PARALLEL_OWNERSHIP.md`：列出所有 open PR / remote branch 的 changed-file ownership、与本 track 的文件级交集、处理策略。规则：
...
- `src/geospatial/io/**fabric**`
- `src/operators/io/*cube*`
- `src/cli/*data*`
- `tests/*fabric*`
- `docs/io/**`
...
**默认 read-only / 避免并发冲突：**
- D19 dataset store
- G01 runtime internals except public contracts
...
最终产品不是"写了一批代码"，而是：**对象存储缓存、离线镜像、GDAL多维数组与云原生查询升级**，并且其 authority、failure semantics、resource bounds、provenance、tests、docs、agent/CLI/GUI surface（适用时）相互一致。先证明已有能力和真实缺口，再实现；对所有科学公式/坐标/单位/时间/NoData 语义采用独立真值，不允许测试复用被测实现来制造绿灯。

## Work packages

| ID | Package | Required deliverables |
|---|---|---|
| A | 统一对象 key/identity | http/s3/gs/az 的 canonical asset token、etag/version/generation、credential-independent cache key。 |
| B | 对象存储 range cache | 将 range cache 扩到 VSI object paths，cache poisoning/credential separation/eviction/corruption 自愈。 |
| C | 真正 offline mirror replay | 提前物化 token→local artifact index；断网时不做任何网络 identity probe；完整性与过期策略。 |
| D | GDAL multidim cube | MDArray dimension inventory、time/y/x/band slicing、dtype/NoData/CRS、窗口 materialization。 |
| E | multidim chunk planner | 维度切片、cost/bytes、million+ logical chunks、不物化全计划。 |
| F | prefetch & locality planner | 访问模式驱动的 range/chunk prefetch、budget/cancel、mirror/cache coordination。 |
| G | CLI/operator parity | catalog/cube inspect/plan/window/prefetch/mirror 的统一 intent/schema。 |
| H | loopback cloud tests | S3-compatible loopback + forced offline + corruption + 100k records + million chunks。 |
...
1. forced-offline 模式下远程 mirror replay 零网络访问
2. 跨协议相同对象 identity 规则不把凭据写入 cache key/log
3. multidim slice known-answer 与 GDAL reference 一致
4. fabric suites 连续两次通过
...
```bash
git worktree add ../exp-rs-cloud-data-fabric-11 -b zcode/cloud-data-fabric-11 origin/master
cd ../exp-rs-cloud-data-fabric-11
```

1. 立即把本 GOAL 原文保存到 `.planning/cloud-data-fabric-11/GOAL.md`，创建 `PLAN.md`、`BASELINE.md`、`DECISIONS.md`、`EVIDENCE.md`、`REVIEW_LOG.md`、`PR_BODY.md`、`PARALLEL_OWNERSHIP.md`、`TEST_MATRIX.md`。
2. 按 repo 现有模式让 `.planning/cloud-data-fabric-11/` 可被 git 跟踪；用 `git check-ignore -v` 验证。不要大改 `.gitignore`。
...
8. `git push -u origin zcode/cloud-data-fabric-11`；禁止 force push。
9. 创建独立 PR：`gh pr create --base master --head zcode/cloud-data-fabric-11 --body-file .planning/cloud-data-fabric-11/PR_BODY.md ...`。不要 merge，不等待线上 checks。
