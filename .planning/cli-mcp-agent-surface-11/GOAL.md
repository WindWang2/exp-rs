...
平台 GUI/CLI/MCP/Pi 已共用大量 operator/tool registry，但随着 10.0 surface 膨胀，仍需要系统性验证 discovery/schema/execute/progress/cancel/error/result 的 parity 与长任务可操作性。
...
建立 `.planning/cli-mcp-agent-surface-11/PARALLEL_OWNERSHIP.md`：列出所有 open PR / remote branch 的 changed-file ownership、与本 track 的文件级交集、处理策略。规则：
...
**默认 read-only / 避免并发冲突：**
- src/agent/harness/** owned by F08 except public tool interfaces
- D18/D19 implementation paths
...
最终产品不是“写了一批代码”，而是：**算子发现、schema、批处理、进度取消与MCP协议面统一**，并且其 authority、failure semantics、resource bounds、provenance、tests、docs、agent/CLI/GUI surface（适用时）相互一致。先证明已有能力和真实缺口，再实现；对所有科学公式/坐标/单位/时间/NoData 语义采用独立真值，不允许测试复用被测实现来制造绿灯。

## Work packages

| ID | Package | Required deliverables |
|---|---|---|
| A | surface census | 所有 operator/tool/command 的 availability、schema、result、help、capability 来源矩阵。 |
| B | CLI batch manifest | 多任务 JSON/JSONL manifest、variables、fail-fast/continue、result index、exit code taxonomy。 |
| C | progress/cancel protocol | 长任务结构化 progress、cancel token、terminal state、stderr/stdout separation。 |
| D | MCP session robustness | stdio framing、request ids、bounded payload、timeout、cancel、child lifecycle、reconnect semantics。 |
| E | large result handles | 不把大数组塞 JSON；artifact refs/page/query/cursor contract。 |
| F | schema parity | CLI --schema / MCP tools/list / Pi registration / help 从同 source projection，drift gate。 |
| G | security & redaction | path/credential/env/secret 不进 logs/results，input bounds。 |
| H | protocol E2E | 真实 binary headless：list→schema→run→progress→cancel→artifact，重复两次。 |
...
1. 同一算子在 CLI/MCP/Pi 的参数 schema 语义一致
2. 长任务可取消且终态唯一
3. 大结果不突破 payload hard cap
4. CLI/MCP targeted E2E 两次通过
...
```bash
git worktree add ../exp-rs-cli-mcp-agent-surface-11 -b zcode/cli-mcp-agent-surface-11 origin/master
cd ../exp-rs-cli-mcp-agent-surface-11
```

1. 立即把本 GOAL 原文保存到 `.planning/cli-mcp-agent-surface-11/GOAL.md`，创建 `PLAN.md`、`BASELINE.md`、`DECISIONS.md`、`EVIDENCE.md`、`REVIEW_LOG.md`、`PR_BODY.md`、`PARALLEL_OWNERSHIP.md`、`TEST_MATRIX.md`。
2. 按 repo 现有模式让 `.planning/cli-mcp-agent-surface-11/` 可被 git 跟踪；用 `git check-ignore -v` 验证。不要大改 `.gitignore`。
...
8. `git push -u origin zcode/cli-mcp-agent-surface-11`；禁止 force push。
9. 创建独立 PR：`gh pr create --base master --head zcode/cli-mcp-agent-surface-11 --body-file .planning/cli-mcp-agent-surface-11/PR_BODY.md ...`。不要 merge，不等待线上 checks。
