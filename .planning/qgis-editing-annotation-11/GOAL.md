# /goal — F11 · QGIS Editing, Annotation & ROI Platform 11.0

/goal  target-agent=zcode  model=GLM-5.3-flash  budget=500000000  subagents<=2  ci=none  autonomy=full  defaults=best

> **Mission:** 遥感样本/ROI/矢量绘制、拓扑、吸附和空间编辑基础升级
> **Target model:** GLM-5.3-flash（长跑大预算；以严格 Oracle/ledger 防止低质量漂移）
> **Branch:** `zcode/qgis-editing-annotation-11`
> **Worktree:** `../exp-rs-qgis-editing-annotation-11`
...
平台大量分析已能产出结果，但用户侧稳定的地图交互编辑、ROI/样本绘制、拓扑/吸附、撤销重做和算法/Agent 可感知的编辑状态仍值得形成独立基础层。
...
建立 `.planning/qgis-editing-annotation-11/PARALLEL_OWNERSHIP.md`：列出所有 open PR / remote branch 的 changed-file ownership、与本 track 的文件级交集、处理策略。规则：
...
- `src/analysis/**roi**`
- `tests/*editing*`
- `docs/workbench/editing*`

**默认 read-only / 避免并发冲突：**
- vendored QGIS core/gui unless a proven minimal upstream-compatible patch is unavoidable
- src/app/main_window_* and mission files while #991 open
- D19 dataset internals
...
最终产品不是"写了一批代码"，而是：**遥感样本/ROI/矢量绘制、拓扑、吸附和空间编辑基础升级**，并且其 authority、failure semantics、resource bounds、provenance、tests、docs、agent/CLI/GUI surface（适用时）相互一致。先证明已有能力和真实缺口，再实现；对所有科学公式/坐标/单位/时间/NoData 语义采用独立真值，不允许测试复用被测实现来制造绿灯。

## Work packages

| ID | Package | Required deliverables |
|---|---|---|
| A | EditSession authority | layer/feature/geometry edit state、dirty/commit/rollback、undo/redo、selection、lock。 |
| B | map tools | point/box/polygon/freehand/brush/erase/split/reshape，QGIS 原生 map tool 优先。 |
| C | snapping/topology | vertex/segment snap、self-intersection、ring、multipart、validity repair preview，不静默改几何。 |
| D | ROI/sample semantics | class label/attributes、raster pixel footprint、CRS transforms、NoData/edge、统计预览。 |
| E | large editing | 十万 feature 索引、incremental render、selection bounds、background stats cancel。 |
| F | agent/command awareness | 只暴露稳定 edit state/selection facts，写操作仍走明确 command，不让 agent 绕过 undo/permission。 |
| G | persistence/interchange | GeoPackage/GeoJSON/temporary layer、project reopen、atomic save。 |
| H | offscreen interaction tests | mouse/key event、undo、snapping、CRS、layer deletion、project close。 |
...
1. 编辑动作全部可 undo/redo 且 project/layer 生命周期无悬挂
2. 几何 validity 问题不静默吞掉
3. 十万 feature 场景不做全量 UI 重建
4. editing targeted tests 两次通过
...
```bash
git worktree add ../exp-rs-qgis-editing-annotation-11 -b zcode/qgis-editing-annotation-11 origin/master
cd ../exp-rs-qgis-editing-annotation-11
```

1. 立即把本 GOAL 原文保存到 `.planning/qgis-editing-annotation-11/GOAL.md`，创建 `PLAN.md`、`BASELINE.md`、`DECISIONS.md`、`EVIDENCE.md`、`REVIEW_LOG.md`、`PR_BODY.md`、`PARALLEL_OWNERSHIP.md`、`TEST_MATRIX.md`。
2. 按 repo 现有模式让 `.planning/qgis-editing-annotation-11/` 可被 git 跟踪；用 `git check-ignore -v` 验证。不要大改 `.gitignore`。
...
8. `git push -u origin zcode/qgis-editing-annotation-11`；禁止 force push。
9. 创建独立 PR：`gh pr create --base master --head zcode/qgis-editing-annotation-11 --body-file .planning/qgis-editing-annotation-11/PR_BODY.md ...`。不要 merge，不等待线上 checks。
