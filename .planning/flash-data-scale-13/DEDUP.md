...
2. **`DataManager::findByPath` is a full scan with per-record identity work**
   (`src/data/data_manager.cpp:821-872`) — the fix makes a probe an O(#shards) key-index
   walk, not a strict O(1) (see ADR 0166) — per record it allocates a `QStringList`
...
## Milestone overlap re-check (pre-review, protocol §6)

`git fetch origin --prune` — `origin/master` still at `79adfe78a1` (no drift since
Phase 0). `scripts/dev/overlap_scan.py --from-HEAD` findings:

| overlapping branch | shared paths | semantic conflict? |
|---|---|---|
| `agent/flash-temporal-phenology-12` (PR #1135, OPEN) | `benchmarks/observatory/*`, `benchmarks/perf-observatory-baseline.md` | none — derived artifacts, regenerated by the same command; last-writer-wins documented |
| `agent/glm53-desktop-lifecycle` (12 commits ahead, 271 behind, no PR) | `src/app/panels/workspace_browser_panel.cpp` | none — its hunk is the `resetQuery` #1056 fix (already merged into master, so the branch's copy is residue); this branch changes `resetQuery`/`fetchMore`/`canFetchMore` for cursor paging (textual conflict at most) |
| `fix/review-issues-1033-1056` (1 commit ahead, 271 behind, no PR) | `src/data/governance/governance_store.cpp` | none — its hunks are in the mutation paths (`removeAsset`, `upsertDataset`, `upsertResult`, `addLineageEdges`, …); this branch rewrites `query()` and adds schema indexes (textual conflict at most) |
| all branches | `tests/CMakeLists.txt` | none — tail-append convention |

No pivot required: no work package of this track was pre-empted.
