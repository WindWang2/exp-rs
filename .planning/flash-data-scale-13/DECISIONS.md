...
**Tradeoff accepted**: per-mutation cost is O(K) shard-copy instead of O(N); with K = 256 that is ~10 µs, i.e. 100k registrations ≈ 1 s versus minutes before. Erase from a sealed shard costs O(K + #shards) — acceptable (unload/reap are rare; `reapSessionTemporaries` over 100k assets is ≈ 2×10⁷ pointer copies).
...
**Adopted: C.** Each shard carries `QHash<QString, QVector<PathKeyEntry>> byKey` where `PathKeyEntry = {seq, AssetId}`; `seq` is the record's insertion sequence so the first match in insertion order wins exactly as the legacy scan did. Key tiers per record: alias spellings (`virtualPathAliases`), the raw `canonicalSource`, the absolute path, and the canonical (symlink-resolved) path. Virtual/remote records contribute alias + raw keys only; path tiers are inserted only for non-virtual records, mirroring the legacy asymmetry. Query keys are computed once per probe (one `QFileInfo` resolution of the query) and looked up in every shard oldest→newest; the walk stops at the first shard with a hit (later shards have higher seqs by construction).
