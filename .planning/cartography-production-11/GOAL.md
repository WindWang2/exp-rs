...
Workbench 10.0 已让 cartography compose/preflight/repair/export 四路可达，但已知 CartographyDock 同步执行、缺 headless preset E2E；制图平台仍可向 atlas、多页、模板组合、导出确定性和生产质量控制深化。
...
建立 `.planning/cartography-production-11/PARALLEL_OWNERSHIP.md`：列出所有 open PR / remote branch 的 changed-file ownership、与本 track 的文件级交集、处理策略。规则：
...
最终产品不是"写了一批代码"，而是：**异步制图生产、地图集、多页排版、模板治理与可重复导出**，并且其 authority、failure semantics、resource bounds、provenance、tests、docs、agent/CLI/GUI surface（适用时）相互一致。先证明已有能力和真实缺口，再实现；对所有科学公式/坐标/单位/时间/NoData 语义采用独立真值，不允许测试复用被测实现来制造绿灯。

## Work packages

| ID | Package | Required deliverables |
|---|---|---|
| A | 异步 cartography job | 通过既有 TaskCenter/public execution seam 后台化 compose/preflight/repair/export；取消、进度、atomic publish。 |
| B | Atlas/series | 基于 feature/region/time 生成多页 MapSpec/atlas，页级变量、extent、标题、页码、索引。 |
| C | 模板/组件 11 | 模板继承、design token、legend/north arrow/scale/title/data source 等组件约束与版本迁移。 |
| D | 布局质量 solver | 重叠、越界、最小字号、图例截断、对齐/留白、可读性规则；repair 有界且留台账。 |
| E | 数据驱动图表/注记 | 统计图表、动态文本、annotation 与 map context 绑定，输出 provenance。 |
| F | 确定性导出 | PDF/SVG/PNG metadata、字体策略、dpi、色彩空间、hash/evidence，环境差异诚实报告。 |
| G | headless preset E2E | CLI/operator/agent 走同引擎，真实 MapSpec fixture 从数据产物到导出。 |
| H | 模板 corpus QA | 所有 shipped template instantiate→validate→preflight→repair→export smoke。 |
...
1. GUI/CLI/agent/operator 不存在第二套 compose/export 逻辑
2. atlas 页数/extent/变量有可机器验证 fixture
3. 取消/失败无半成品，repair 循环有界
4. mapspec/cartography suites 两次通过
...
```bash
git worktree add ../exp-rs-cartography-production-11 -b zcode/cartography-production-11 origin/master
cd ../exp-rs-cartography-production-11
```

1. 立即把本 GOAL 原文保存到 `.planning/cartography-production-11/GOAL.md`，创建 `PLAN.md`、`BASELINE.md`、`DECISIONS.md`、`EVIDENCE.md`、`REVIEW_LOG.md`、`PR_BODY.md`、`PARALLEL_OWNERSHIP.md`、`TEST_MATRIX.md`。
2. 按 repo 现有模式让 `.planning/cartography-production-11/` 可被 git 跟踪；用 `git check-ignore -v` 验证。不要大改 `.gitignore`。
...
8. `git push -u origin zcode/cartography-production-11`；禁止 force push。
9. 创建独立 PR：`gh pr create --base master --head zcode/cartography-production-11 --body-file .planning/cartography-production-11/PR_BODY.md ...`。不要 merge，不等待线上 checks。
